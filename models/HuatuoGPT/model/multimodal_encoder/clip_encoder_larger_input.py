...
class CLIPVisionEmbeddingsLargerInput(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        patch_embeds = patch_embeds.flatten(
            2
        ).transpose(
            1, 2
        )  # (bsz, emb_dim, p_size=24, p_size=24) -> (bsz, emb_dim, 24*24=576) -> (bsz, 576, emb_dim)
        ...
        embeddings = embeddings + self.interpolate_pos_encoding(
            embeddings, pixel_values.shape[2], pixel_values.shape[3]
        )
        # use interpolation
    ...
    def interpolate_pos_encoding(
        ...
        num_positions = (
            positional_embedding.shape[1] - 1
        )  # number of positions of inputs

        if num_patches == num_positions and height == width:  # if it matches
            return positional_embedding

        assert num_positions < num_patches, (
            f"there must be an increased number of positions, got {num_positions} > {num_patches}"
        )
        ...
        patch_pos_embed = patch_pos_embed.permute(0, 2, 3, 1).view(
            bsz, -1, dim
        )  # (bsz, dim, h, w) -> (bsz, h, w, dim) -> (bsz, seqlen=h*w, dim)
...
class CLIPVisionTower(nn.Module):
    ...
    def load_model(self):
        ...
        if "clip" in self.vision_tower_name.lower():
            ...
            self.vision_tower.vision_model.embeddings = CLIPVisionEmbeddingsLargerInput(
...
if __name__ == "__main__":
    vision_tower = CLIPVisionTower(
        "/wangbenyou/guimingchen/models/clip_vit_large_patch14_336", None
    )
    ...
    image = Image.open(
        "/wangbenyou/guimingchen/workspaces/vllm/data/images_miniset/openai_wewb_screenshot.png"
    ).convert("RGB")
