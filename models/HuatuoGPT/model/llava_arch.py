    ...
    def prepare_inputs_labels_for_multimodal_new(
        ...
        if not self.training:  # TODO: check this out!!
            ...
            if (
                ...
                or images is None
                or (isinstance(input_ids, torch.Tensor) and input_ids.shape[1] == 1)
            ):
                if (
                    past_key_values is not None
                    and vision_tower is not None
                    ...
                    if image_feature.shape[0] > 1:
                        ...
                        if "anyres" in image_aspect_ratio:
                            ...
                            num_patch_width, num_patch_height = (
                                big_width // patch_size,
                                big_height // patch_size,
                            )
        ...
        if attention_mask is None:
            # attention_mask = torch.ones_like(input_ids, dtype=torch.bool)
            attention_mask = [
                torch.tensor([1] * l).to(input_ids).bool()
                for l in map(len, [ip for ip in input_ids])
            ]
        else:
            # attention_mask = attention_mask.bool()
        ...
        if labels is None:
            labels = [
                torch.tensor([IGNORE_INDEX] * l).to(input_ids[0])
                for l in map(len, [ip for ip in input_ids])
            ]
            # labels = torch.full_like(input_ids, IGNORE_INDEX)
        else:
        ...
        assert cur_image_idx == len(image_features), (
            f"wrong!! cur_image_idx != len(image_features) cur_image_idx:{cur_image_idx} len(image_features):{len(image_features)}"
        )
        ...
        attention_mask = torch.zeros(
            (batch_size, max_len), dtype=torch.bool, device=attention_mask[0].device
        )
        position_ids = torch.zeros(
            (batch_size, max_len), dtype=torch.long, device=attention_mask[0].device
        )
        ...
        for i, (cur_new_embed, cur_new_labels) in enumerate(
            ...
            if (
                getattr(self.config, "tokenizer_padding_side", "right") == "left"
