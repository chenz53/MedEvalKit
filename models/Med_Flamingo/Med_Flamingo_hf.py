...
class Med_Flamingo:
    def __init__(self, model_path, args):
        ...
        self.model.load_state_dict(
            torch.load(self.checkpoint_path, map_location=self.device), strict=False
        )
    ...
    def process_batch_messages(self, messages_list):
        ...
        if "image" in messages_list[0]:
            ...
            for images_for_one_example in batch_images:
                ...
                processed_images = self.processor.preprocess_images(
                    images_for_one_example
                )
                batch_pixels.append(processed_images)

            pixels = torch.stack(batch_pixels, dim=0)  # Now 'b' will be the batch size
            pixels = repeat(
                pixels, "b N c h w -> b N T c h w", T=1
            )  # If T is always 1, no change needed
    ...
    def generate_output(self, messages):
        ...
        response = self.processor.tokenizer.decode(
            get_before_first_next_line(
                generated_text[0][tokenized_data["input_ids"].shape[1] :]
            )
        )
    ...
    def generate_outputs(self, messages_list):
        ...
        results = [
            self.processor.tokenizer.decode(
                text[tokenized_data["input_ids"].shape[1] :]
            )
