...
class HealthGPT:
    def __init__(self, model_path, args):
        ...
        num_new_tokens = add_special_tokens_and_resize_model(
            self.tokenizer, self.llm, 8192
        )
        print(f"Number of new tokens added for unified task: {num_new_tokens}")

        com_vision_args.model_name_or_path = model_path
        com_vision_args.vision_tower = (
            "/mnt/workspace/workgroup_dev/longli/models/hub/clip-vit-large-patch14-336"
        )
        ...
        self.llm = load_weights(
            self.llm,
            "/mnt/workspace/workgroup_dev/longli/models/hub/HealthGPT-L14/com_hlora_weights_phi4.bin",
        )
    ...
    def generate_output(self, messages):
        ...
        if imgs:
            ...
            imgs = [
                expand2square(
                    ...
                    tuple(
                        int(x * 255)
                        for x in self.llm.get_vision_tower().image_processor.image_mean
                    ),
                )
                for img in imgs
            ]
            imgs = (
                self.llm.get_vision_tower()
                .image_processor.preprocess(imgs, return_tensors="pt")["pixel_values"]
                .to(dtype=torch.float16, device="cuda", non_blocking=True)
            )
        ...
        with torch.inference_mode():
            ...
            output_ids = self.llm.base_model.model.generate(
                input_ids,
                images=imgs,
                do_sample=do_sample,
                num_beams=5,
