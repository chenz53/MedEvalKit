...
def infer():
    ...
    parser.add_argument(
        "--vit_path", type=str, default="openai/clip-vit-large-patch14-336"
    )
    ...
    model_dtype = (
        torch.float32
        if args.dtype == "FP32"
        else (torch.float16 if args.dtype == "FP16" else torch.bfloat16)
    )
    ...
    num_new_tokens = add_special_tokens_and_resize_model(
        tokenizer, model, args.vq_idx_nums
    )
    ...
    if img_path:
        ...
        image = expand2square(
            ...
            tuple(
                int(x * 255)
                for x in model.get_vision_tower().image_processor.image_mean
            ),
        )
        image_tensor = (
    ...
    response = [
        int(idx) for idx in re.findall(r"\d+", tokenizer.decode(output_ids[0])[:-8])
    ]
    print(f"Q: {question}")
