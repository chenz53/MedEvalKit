...
def add_special_tokens_and_resize_model(tokenizer, model, vq_idx_nums=8192):
    ...
    special_tokens = {
        "additional_special_tokens": ["<start_index>"]
        + index_tokens
        + ["<end_index>"]
        + ["<pixel_newline>"]
    }
...
def load_weights(model, hlora_path, fusion_layer_path=None):
    ...
    if fusion_layer_path:
        ...
        fusion_layer_unexpected_keys = model.load_state_dict(
            fusion_layer_weights, strict=False
        )[1]
        if fusion_layer_unexpected_keys:
            print(
                f"Warning: Unexpected keys in fusion_layer checkpoint: {fusion_layer_unexpected_keys}"
            )
