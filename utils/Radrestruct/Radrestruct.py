...
class Radrestruct(BaseDataset):
    ...
    def load_data(self):
        ...
        id_to_img_mapping_path = os.path.join(
            dataset_path, "id_to_img_mapping_frontal_reports.json"
        )
    ...
    def construct_messages(self, sample, answer_options):
        ...
        for qa_pair_idx, qa_pair in enumerate(qa_pairs):
            ...
            is_reasoning = (
                True if os.environ.get("REASONING", "False") == "True" else False
            )
    ...
    def cal_metrics(self, out_samples):
        metrics = {
            ...
            "single_choice": {"total": 0, "right": 0},
            "multiple_choice": {"total": 0, "right": 0},
        }
        ...
        metrics["total metrics"]["acc"] = (
            metrics["total metrics"]["right"] / metrics["total metrics"]["total"]
        )
        metrics["single_choice"]["acc"] = (
            metrics["single_choice"]["right"] / metrics["single_choice"]["total"]
        )
        metrics["multiple_choice"]["acc"] = (
            metrics["multiple_choice"]["right"] / metrics["multiple_choice"]["total"]
        )
