...
class SLAKE(BaseDataset):
    ...
    def maybe_download_dataset(self):
        if not os.path.exists(self.dataset_path):
            ...
            snapshot_download(
                self.hf_path, local_dir=self.dataset_path, repo_type="dataset"
            )
            self._unzip_img_zip_local(
                local_path=self.dataset_path, zip_filename="imgs.zip"
            )
            self._unzip_img_zip_local(
                local_path=self.dataset_path, zip_filename="KG.zip"
            )
