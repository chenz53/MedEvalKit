...
for dataset in datasets:
    ...
    for sample in tqdm(datas):
        ...
        bleu2 = sentence_bleu(
            [tokenized_golden], tokenized_response, weights=[0.5, 0.5]
        )
        bleu3 = sentence_bleu(
            [tokenized_golden], tokenized_response, weights=[1 / 3, 1 / 3, 1 / 3]
        )
        bleu4 = sentence_bleu(
            [tokenized_golden], tokenized_response, weights=[0.25, 0.25, 0.25, 0.25]
        )
        total_bleu1 += bleu1
        total_bleu2 += bleu2
        total_bleu3 += bleu3
        ...
        meteor_scores = single_meteor_score(
            hypothesis=tokenized_response, reference=tokenized_golden
        )
        total_meteor_scores += meteor_scores

    print("begin to compute cider metrics...")
    cider_score = cider(
        predictions=[pred.lower() for pred in preds], references=reports2
    )
    ...
    green_mean, green_std, green_score_list, summary, result_df = green_scorer(
        reports, preds
    )
