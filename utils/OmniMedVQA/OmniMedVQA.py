...
from ..utils import extract, judge_multi_choice
...
class OmniMedVQA(BaseDataset):
    def __init__(self, model, dataset_path, output_path):
        ...
        self.dataset_path = (
            dataset_path if dataset_path is not None else "./datas/OmniMedVQA"
        )
        self.samples = []
            ...
            if num_chunks > 1:
                for current_messages, current_samples in tqdm(messages_list):
                    for i in range(len(current_messages)):
                        current_messages[i]["image"] = Image.open(
                            current_messages[i]["image"]
                        )
    ...
    def cal_metrics(self, out_samples):
        ...
        metrics = {
            ...
            "question type metrics": question_type_metrics,
            "modality type metrics": modality_type_metrics,
        }
    ...
    def maybe_download_dataset(self):
        if not os.path.exists(self.dataset_path):
            if self.chunk_idx != 0:
                raise ValueError(
                    "Chunk inference is not support for download. Try to run eval.sh insteal of eval_chunked.sh"
                )
            ...
            self._unzip_img_zip_local(
                local_path=os.path.dirname(self.dataset_path),
                zip_filename="OmniMedVQA.zip",
            )
