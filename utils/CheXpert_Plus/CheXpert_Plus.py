...
class CheXpert_Plus(BaseDataset):
    ...
    def maybe_download_dataset(self):
        if not os.path.exists(self.dataset_path):
            raise ValueError(
                f"Dataset path {self.dataset_path} does not exist. Please download from https://aimi.stanford.edu/datasets/chexpert-plus and put it under {self.dataset_path}."
            )
