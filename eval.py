...
def main():
    ...
    parser.add_argument(
        "--eval_datasets",
        type=parse_eval_datasets,
        default=["MMMU"],
        help="name of eval dataset",
    )
    parser.add_argument(
        "--datasets_path", type=str, default="benchmarks", help="path of eval dataset"
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default="eval_results/Qwen2-VL-7B-Instruct",
        help="name of saved json",
    )
    parser.add_argument(
        "--model_name", type=str, default="Qwen2-VL-7B-Instruct", help="name of model"
    )
    ...
    for eval_dataset in tqdm(args.eval_datasets):
        ...
        eval_dataset_path = (
            os.path.join(args.datasets_path, eval_dataset)
            if args.datasets_path != "hf"
            else None
        )
        eval_output_path = os.path.join(args.output_path, eval_dataset)
        os.makedirs(eval_output_path, exist_ok=True)
        benchmark = prepare_benchmark(
            model, eval_dataset, eval_dataset_path, eval_output_path
        )
        benchmark.load_data()
